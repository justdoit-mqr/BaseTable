 ...
 *
 * 2.该类主要提供对编号列的文本渲染以及“批量选择”、"喜欢"列的图标进行渲染。使用委托drawItemPixmap的
 * 方式渲染单元格图片相较于QTableWidgetItem的setIcon()，它可以解决居中问题。相比设置cellWidget它
 * 也有很大的性能优势(设置cellWidget需要事先new出相应的部件对象，当所需部件较多时即占空间又费时间)。
 *
 */
